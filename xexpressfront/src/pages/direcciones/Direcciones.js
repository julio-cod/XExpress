...
import { ApiBuscarDireccionesByDNICliente } from '../../servicios/ApiRest';
...
export default function Direcciones() {
    ...
    const baseUrlBuscarClientesByDNICliente = ApiRestBase  + ApiControladorDireccion + ApiBuscarDireccionesByDNICliente;
    ...
    const [provinciaSeleccionadaReg, setProvinciaSeleccionadaReg] = useState('');
    ...
    useEffect(() => {
        if(modalEditar !==true){
            if (direccionSeleccionada.provincia !== '') {
                setProvinciaSeleccionadaReg(direccionSeleccionada.provincia );
            }
        }
    ...
    const CargarDirecciones = async () => {
        await axios.get(baseUrlListaDirecciones)
            .then(response => {
    ...
    const CargarDireccionesByDNI = async () => {
        if(clienteDigitadoBusc.dni !== '' || clienteDigitadoBusc.dni !== undefined){
            await axios.get(baseUrlBuscarClientesByDNICliente +"/"+ clienteDigitadoBusc.dni)
            .then(response => {
                ...
                if (error.response.status === 401) {
                    ...
                    var Resp = "Error al tratar de buscar direcciones por DNI";
                notifyError(Resp);
                }
            })
        }
        else{
            var Resp = "Digite el DNI";
            notifyError(Resp);
        }
    ...
    return (
        ...
        {/*<button className="btn btn-primary" onClick={() => abrirCerrarModelInsertar()}>Nuevo</button>*/}{" "}
            ...
            <div  className="d-flex" style={{width: '40%', maxWidth:'40%'}}>
                ...
                <input type="text" className="form-control" style={{marginLeft: '2%'}} name="dni" placeholder="DNI" onChange={handleChangeClienteDigitadoBusc} value={clienteDigitadoBusc && clienteDigitadoBusc.dni} onKeyDown={handleKeyDown} />
                
                <button className="btn btn-primary" style={{marginLeft: '2%'}} onClick={() => CargarDireccionesByDNI()}>Buscar</button>
        ...
        <div  >
            <Select className="contenedorSelectModalDireccion" sx={{ width: '30%' }}
                ...
                value={provinciaSeleccionadaReg}
                label="provincia"
